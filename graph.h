...
public:
  ...
  ////////////////////////////////////////////////////////////
  void InsertVertex(VertexType const &v) { outgoining_edges.insert(std::make_pair(v, std::vector<EdgeType>())); }
  ////////////////////////////////////////////////////////////
  void BuildFromEdgeArray(EdgeType *array, size_t const &size) {
    for (size_t i = 0; i < size; ++i) {
      ...
      outgoining_edges[v1];
      outgoining_edges[v2];
      //				outgoining_edges.insert( std::make_pair/*<VertexType,std::vector<EdgeType> >*/( v1,
      //std::vector<EdgeType>())); 				outgoining_edges.insert( std::make_pair/*<VertexType,std::vector<EdgeType> >*/( v2,
      //std::vector<EdgeType>()));
  ...
  VertexType const &GetVertex(size_t id) const {
    ...
    if (it != outgoining_edges.end())
      ...
      throw "cannot find node in the graph";
  }
  ////////////////////////////////////////////////////////////
  typename std::list<EdgeType> const &GetEdges() const { return edges_all; }
  ////////////////////////////////////////////////////////////
  ...
  ////////////////////////////////////////////////////////////
  size_t Size() const { return outgoining_edges.size(); }
  ////////////////////////////////////////////////////////////
  ...
  friend std::ostream &operator<<(std::ostream &os, Graph const &g) {
    typename std::map<VertexType, std::vector<EdgeType>>::const_iterator it = g.outgoining_edges.begin(),
    ...
    for (; it != it_e; ++it) {
      ...
      for (size_t i = 0; i < size; ++i) {
        std::cout << "\t" //<< it->second[i].GetName()
                  << " (" << it->second[i].ID1() << " -> " << it->second[i].ID2() << ")" << std::endl;
      }
    }
    return os;
  }
